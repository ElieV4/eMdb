...
import { useDashboardStats, usePopularTitles, useRecentWatches, useFollowedSeries } from '@/hooks/api/useDashboard';
...
describe('useDashboard', () => {
  ...
  it('fetches recent watches', async () => {
    mockApiFetch.mockResolvedValue({ items: [], total: 0, page: 1, limit: 6, totalPages: 0 });
  ...
  it('fetches popular titles', async () => {
    mockApiFetch.mockResolvedValue({ items: [], total: 0, page: 1, limit: 12, totalPages: 0 });
    const queryClient = createQueryClient();
    const wrapper = createWrapper(queryClient);
