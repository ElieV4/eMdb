...
| **Base de données**                | PostgreSQL (managé via Supabase)         | ✅ Gratuit (free tier)  | 500 Mo DB / 1 Go stockage — projet mis en pause après 1 semaine d'inactivité (voir keep-alive ci-dessous)                                  |
| **Auth**                           | JWT + bcrypt fait maison (NestJS/Passport) | ✅ Gratuit            | Implémentation actuelle du repo (`apps/api/src/auth`) — pas Supabase Auth                                                                  |
| **Queue / cron**                   | BullMQ + Redis (Upstash)                 | ✅ Gratuit (free tier)  | 500k commandes/mois — worker embarqué dans le service API en cloud (voir ci-dessous), séparé en local (docker-compose)                    |
